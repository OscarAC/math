...
\begin{problems}
    ...
    \answer Solving $y = \dfrac{5x+1}{x-2}$ for $x$ gives $x = \dfrac{2y+1}{y-5}$, so
    \[
        f^{-1} : \mathbb{R} - \set{5} \to \mathbb{R} - \set{2}, \qquad f^{-1}(x) = \dfrac{2x+1}{x-5}.
    \]

    \problem The function $f : \mathbb{R} \to (0,\infty)$ defined as $f(x) = e^{x^3+1}$ is bijective. Find its inverse.

    \answer Solving $y = e^{x^3+1}$ for $x$ gives $x = \sqrt[3]{\ln{y}-1}$, so
    \[
        f^{-1} : (0,\infty) \to \mathbb{R}, \qquad f^{-1}(x) = \sqrt[3]{\ln{x}-1}.
    \]
    ...
    \answer Solving $5m+4n = a$ and $4m+3n = b$ for $m$ and $n$ gives $m = -3a+4b$ and $n = 4a-5b$, so
    \[
        f^{-1}(a,b) = (-3a+4b,\; 4a-5b).
    \]
    ...
    \answer Yes. Since $\overline{\overline{X}} = X$, the function $\theta$ is its own inverse: $\theta^{-1}(X) = \overline{X}$.
    ...
    \answer By Exercise 18 of Section 12.2, $f(2k) = k$ for $k \geq 1$ and $f(2k+1) = -k$ for $k \geq 0$. Inverting each case,
    \[
        f^{-1} : \mathbb{Z} \to \mathbb{N}, \qquad
        f^{-1}(c) = \begin{cases}
            2c      & \text{if } c \geq 1,\\
            1 - 2c  & \text{if } c \leq 0.
        \end{cases}
...
\begin{problems}
    ...
    \answer
    \begin{align*}
        f(\set{1,2,3}) &= \set{3,8}, & f(\set{4,5,6,7}) &= \set{1,2,4,6}, & f(\emptyset) &= \emptyset,\\
        f^{-1}(\set{0,5,9}) &= \emptyset, & f^{-1}(\set{0,3,5,9}) &= \set{1,3}. &&
    \end{align*}
    ...
    \answer We can determine how many such functions have this property by firs choosing which 4 of the 8 elements of the domain are sent to 2, which can be done in $\bino{8}{4} = 70$ ways. 
    Then each of the remaining 4 elements must be sent to one of the 6 elements of the codomain other than 3, giving $6^4 = 1296$ possibilities.
    Thus, by the multiplication principle, there are $\binom{8}{4} \cdot 6^4 = 90720$ such functions.

    \problem Given a function $f : A \to B$ and a subset $Y \subseteq B$, is $f(f^{-1}(Y)) = Y$ always true? Prove or give a counterexample.

    \answer No. For a counterexample, let $A = \set{1}$, $B = \set{1,2}$ and $f = \set{(1,1)}$, and take $Y = \set{2}$. No element of $A$ is sent into $Y$, so $f^{-1}(Y) = \emptyset$ and therefore
    \[
        f\big(f^{-1}(Y)\big) = f(\emptyset) = \emptyset \neq \set{2} = Y.
    \]
    ...
    \answer Let $A = \set{1,2}$, $B = \set{1}$ and $f = \set{(1,1),(2,1)}$, and take $W = \set{1}$ and $X = \set{2}$. Then $W \cap X = \emptyset$, so
    \[
        f(W \cap X) = f(\emptyset) = \emptyset,
    \]
    whereas $f(W) = \set{1}$ and $f(X) = \set{1}$, so $f(W) \cap f(X) = \set{1}$. Thus $f(W \cap X) \neq f(W) \cap f(X)$.

    \problem Given $f : A \to B$ and subsets $Y, Z \subseteq B$, prove $f^{-1}(Y \cap Z) = f^{-1}(Y) \cap f^{-1}(Z)$.
    \begin{proof}
        Let $x \in A$. Recall that $x \in f^{-1}(S)$ means exactly that $f(x) \in S$. Then
        \begin{align*}
            x \in f^{-1}(Y \cap Z)
                &\iff f(x) \in Y \cap Z\\
                &\iff f(x) \in Y \text{ and } f(x) \in Z\\
                &\iff x \in f^{-1}(Y) \text{ and } x \in f^{-1}(Z)\\
                &\iff x \in f^{-1}(Y) \cap f^{-1}(Z).
        \end{align*}
        Since an element belongs to one set if and only if it belongs to the other, the two sets are equal.
    \end{proof}
    ...
    \begin{proof}
        ...
        Suppose $f$ is injective and let $X \subseteq A$. If $x \in X$ then $f(x) \in f(X)$, so $x \in f^{-1}(f(X))$; this gives $X \subseteq f^{-1}(f(X))$.
        Conversely, suppose $a \in f^{-1}(f(X))$. Then $f(a) \in f(X)$, so $f(a) = f(x)$ for some $x \in X$. Since $f$ is injective, $a = x$, hence $a \in X$. This gives $f^{-1}(f(X)) \subseteq X$, so the two sets are equal.

        Now suppose $X = f^{-1}(f(X))$ for every $X \subseteq A$, and let $a, b \in A$ with $f(a) = f(b)$. Applying the hypothesis to $X = \set{a}$ gives $f^{-1}(f(\set{a})) = \set{a}$. But $f(b) = f(a) \in f(\set{a})$, so $b \in f^{-1}(f(\set{a})) = \set{a}$, which forces $b = a$. Therefore $f$ is injective.

        Next, we prove $f$ is surjective.

        Suppose $f$ is surjective and let $Y \subseteq B$. 
        If $y \in f(f^{-1}(Y))$, then $y = f(x)$ for some $x \in f^{-1}(Y)$, and $x \in f^{-1}(Y)$ means $f(x) \in Y$, so $y \in Y$; this gives $f(f^{-1}(Y)) \subseteq Y$. 
        Conversely, let $y \in Y$. Since $f$ is surjective there is an $x \in A$ with $f(x) = y$. As $f(x) = y \in Y$, we have $x \in f^{-1}(Y)$, and therefore $y = f(x) \in f(f^{-1}(Y))$. This gives $Y \subseteq f(f^{-1}(Y))$, so the two sets are equal.

        Now suppose $f(f^{-1}(Y)) = Y$ for every $Y \subseteq B$, and let $b \in B$. Applying the hypothesis to $Y = \set{b}$ gives $f(f^{-1}(\set{b})) = \set{b}$. In particular this set is nonempty, so $f^{-1}(\set{b}) \neq \emptyset$. Choosing any $x \in f^{-1}(\set{b})$ gives $f(x) \in \set{b}$, that is, $f(x) = b$. Therefore $f$ is surjective.
    \end{proof}

    \problem Let $f : A \to B$ be a function, and $Y \subseteq B$. Prove or disprove: $f^{-1}\big(f(f^{-1}(Y))\big) = f^{-1}(Y)$.
    \begin{proof}
        The statement is true. Let $W = f^{-1}(Y)$.

        First we show $W \subseteq f^{-1}(f(W))$. 
        If $x \in W$ then $f(x) \in f(W)$, thus $x \in f^{-1}(f(W))$.

        Next we show $f^{-1}(f(W)) \subseteq W$.
        Suppose $x \in f^{-1}(f(W))$, so $f(x) \in f(W)$.
        Then $f(x) = f(w)$ for some $w \in W$.
        Since $w \in W = f^{-1}(Y)$, we have $f(w) \in Y$, and therefore $f(x) = f(w) \in Y$.
        This means $x \in f^{-1}(Y) = W$.

        The two inclusions give $f^{-1}\big(f(f^{-1}(Y))\big) = f^{-1}(Y)$.
    \end{proof}
\end{problems}
