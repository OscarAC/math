...
\begin{problems}
    ...
    \answer Observe that, by properties of the function, $lne^x = x$ and $e^{lnx} = x$ for all $x \in (0,\infty)$. 
    Now, suppose $ln(m) = ln(n)$ for some $m,n \in (0,\infty)$.
    This means $e^{ln{m}} = e^{ln{n}}$, that is $m=n$.
