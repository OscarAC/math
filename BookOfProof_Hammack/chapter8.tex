...
\begin{problems}
    ...
    \begin{proof}
        Observe the following sequence of equalities.
        \begin{align*}
            A \times (B - C) & = \set{(x,y) : x \in A \land y \in B-C} && \text{($\times$ def.)}\\ 
                             ...
                             & = \set{(x,y) : (x,y) \in A \times B \land (x,y) \notin A \times C} && \text{($\times$ def.)}\\
                             & = \set{(x,y) : (x,y) \in (A \times B) - (A \times C)} && \text{($-$ def.)}\\
                             & = (A \times B) - (A \times C) && \text{(set-builder def.)}
        \end{align*}
    \end{proof}
